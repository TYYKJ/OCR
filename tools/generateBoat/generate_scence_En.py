...
class GenPlateScene:
    ...
    def __init__(self, font_ch_path: str, font_en_path: str, bg_folder: str):
        ...
        """
        self.fontC = ImageFont.truetype(font_ch_path, 45, 0)
        self.fontE = ImageFont.truetype(font_en_path, 28, 0)
        self.img = np.array(Image.new("RGB", (100, 32), (255, 255, 255)))
        self.bg = cv2.resize(cv2.imread(TEMPLATE_IMAGE), (100, 32))
        self.no_plates_path = []
    ...
    def draw(self, boat_num: str) -> np.ndarray:
        ...
        """
        offset = 2
        self.img[0:32, offset + 8:offset + 8 + 18] = generate_en_characters(self.fontE, boat_num[0])
        self.img[0:32, offset + 8 + 18:offset + 8 + 18 + 18] = generate_en_characters(self.fontE, boat_num[1])

        for i in range(3):
            base = offset + 8 + 18 + 18 + i * 18
            self.img[0:32, base:base + 18] = generate_en_characters(self.fontE, boat_num[i + 2])
        return self.img
    ...
    def generate(self, text: str) -> tuple:
        ...
        # com = AddGauss(com, 1 + random_seed(1))  # 加高斯平滑
